package com.example.server.handler_;
...
import com.example.server.service_.PersonIDService;
import com.example.server.service_.PersonService;
            ...
            if (exchange.getRequestMethod().toLowerCase().equals("get")) {
                ...
                if (reqHeaders.containsKey("Authorization")) {
                    ...
                    if (urlElements.length == 2) {
                        ...
                        // Send the HTTP response to the client
                        if ("Error: AuthToken not valid".equals(response.getMessage()) ||
                                "Error: Requested person does not belong to this user".equals(response.getMessage()) ||
                                "Error: PersonID not valid".equals(response.getMessage())) {
                            exchange.sendResponseHeaders(HttpURLConnection.HTTP_BAD_REQUEST, 0);
                        }
                        else {
                            exchange.sendResponseHeaders(HttpURLConnection.HTTP_OK, 0);
